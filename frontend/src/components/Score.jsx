...
const Score = () => {
  ...
  const fetchTotalData = async () => {
    try {
      const response = await fetch("https://nexus-22i4.onrender.com/get-user-score", {
        method: "GET",
        credentials: "include"
      });
      ...
      const savedTotalScore = parseInt(localStorage.getItem("total_score")) || 0;
      setTotalData({
  ...
  const getTotalScorePercentage = () => {
    ...
    const totalPossibleScore = Object.values(allRoundsData).reduce((sum, round) => {
        ...
        {currentRound !== 'total' && roundData ? (
          ...
          <>
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-gray-400">
                Multiplier: ×{roundData.scoreMultiplier} | 
                Questions: {roundData.totalQuestions}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                <div className="text-4xl font-bold mb-2">
                  <span className={getScoreColor(roundPercentage)}>{roundData.score}</span>
                  <span className="text-gray-400">/{roundData.totalQuestions * roundData.scoreMultiplier}</span>
                </div>
                <p className="text-gray-300 text-sm uppercase tracking-wide">Round Score</p>
              </div>

              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                <div className={`text-4xl font-bold mb-2 ${getScoreColor(roundPercentage)}`}>
                  {roundPercentage}%
                </div>
                <p className="text-gray-300 text-sm uppercase tracking-wide">Accuracy</p>
              </div>
              ...
              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                ...
                <p className="text-gray-300 text-sm uppercase tracking-wide">Questions Attempted</p>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-gray-300 font-medium">Round Progress</span>
                <span className="text-gray-300">{roundData.questionAttempt} / {roundData.totalQuestions}</span>
              </div>
              <div className="w-full bg-gray-700 rounded-full h-3">
                <div 
                  className="bg-gradient-to-r from-cyan-400 to-blue-500 h-3 rounded-full transition-all duration-1000 ease-out"
                  style={{ width: `${roundData.totalQuestions > 0 ? (roundData.questionAttempt / roundData.totalQuestions) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-gray-800 rounded-xl p-4 border border-gray-600">
                <div className="text-2xl font-bold text-red-400 mb-1">
                  {roundData.questionAttempt - Math.round(roundData.score / roundData.scoreMultiplier)}
                </div>
          ...
          <>
            <div className="text-center mb-4">
              ...
              <p className="text-gray-400">
                Rounds Completed: {totalData.roundsCompleted.length} / {totalData.totalRounds}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                <div className="text-4xl font-bold mb-2">
                  <span className={getScoreColor(totalPercentage)}>{totalData.totalScore}</span>
                </div>
                <p className="text-gray-300 text-sm uppercase tracking-wide">Total Score</p>
              </div>

              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                <div className={`text-4xl font-bold mb-2 ${getScoreColor(totalPercentage)}`}>
                  {totalPercentage}%
                </div>
                <p className="text-gray-300 text-sm uppercase tracking-wide">Overall Accuracy</p>
              </div>
              ...
              <div className="bg-gray-800 rounded-2xl p-6 text-center border border-gray-600">
                ...
                <p className="text-gray-300 text-sm uppercase tracking-wide">Total Questions</p>
              </div>
            </div>

            {/* Round Breakdown */}
            <div className="4">
              <h3 className="text-xl font-bold mb-4 text-center">Round Breakdown</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[1, 2, 3, 4].map((round) => {
                  ...
                  return (
                    <div key={round} className={`bg-gray-800 rounded-xl p-4 border ${
                        ...
                        {roundInfo ? (
                          <>
                            ...
                            <p className="text-gray-400 text-sm">
                              {roundInfo.questionAttempt} / {roundInfo.totalQuestions} questions
                            </p>
